...
$$
这里，$P\left(Z \mid Y, \theta^{(i)}\right)$是在给定观测数据$Y$和当前的参数估计$\theta^{(i)}$下 隐变量$Z$的条件概率分布。
...
$$
\begin{equation}\theta^{(i+1)}=\arg \max _{\theta} Q\left(\theta, \theta^{(i)}\right)\end{equation}
$$


（例如在第一小节抛硬币的例子中，step2就是在step1的基础上，不断更新每个硬币出现正反面的概率数值）
...
事实上，EM我们是希望通过迭代逐步近似极大化$L(\theta)$的。假设在第$i$次迭代后$\theta$的估计值是$\theta^{(i)}$。那么我们希望$L(\theta) >L(\theta^{(i)})$，并逐步达到极大值。
...
在对数据进行建模的过程中，我们常常假设数据服从高斯分布。但是高斯模型是关于均值$\mu$对称的。从下面数据图可看出，若我们假设采用单个高斯模型来进行建模，这显然是不符合所采集到的数据特征。
...
那么此时的模型参数应该包含三个高斯分布的参数，为$\theta=\{\mu_{1},\mu_{2},\mu_{3},\Sigma_{1},\Sigma_{2},\Sigma_{3}\,\alpha_{1},\alpha_{2}\} $

其中$\alpha_{1},\alpha_{2},\alpha_{3}$分别表示三个独立的高斯分布的权重。因为$\sum_{i=1}^{3}{\alpha_{i}=1}$，因此确定了前面两个模型的权重，那么第三个模型的权重也可以得到了。



**补充知识：**

1、在多个高斯分布叠加数据模型中，的那么一个随机的样本数据出现的概率应该为


$$
\begin{equation}P\left(x_{i} \mid \bar{\theta}\right)=\sum_{l=1}^{k} \alpha_{l} N\left(\mu_{l} ,\Sigma_{l}\right)\end{equation}
$$


2、混合高斯模型(GMM)可以用来做聚类算法。GMM模型在应用于数据聚类时是软聚类算法。

![](../image/EM聚类示意图.gif)

Q:为什么GMM模型在应用于数据聚类时是软聚类算法？

A:GMM模型会计算样本数据属于各个类别的概率，然后改样本数据属于哪个类别的概率最大，那么那个类别就是该样本数据的聚类类别。虽然样本数据属于某个类别的概率较小，但是GMM模型并不否定这个样本数据也属于其他类别。
